...
void myinit(void) {
	...
	glBufferData(GL_ARRAY_BUFFER, 2 * 4 * 4 * sizeof(GLfloat),
		NULL, GL_STATIC_DRAW);
	glBufferSubData(GL_ARRAY_BUFFER, 0, 4 * 4 * sizeof(GLfloat),
		vertices);
	glBufferSubData(GL_ARRAY_BUFFER, 4 * 4 * sizeof(GLfloat),
		4 * 4 * sizeof(GLfloat),
		colors);
